...
import { FiCar, FiMapPin, FiZap, FiDollarSign, FiBarChart2 } from "react-icons/fi";
  ...
  return (
    <div className="max-w-7xl mx-auto p-6 space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <h2 className="text-2xl font-bold text-slate-800">Vehicle Dashboard</h2>
        <div className="w-48">
          <CustomSelect
            value={timeFilter}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <div className="bg-white p-6 rounded-xl border border-slate-100 shadow-sm flex items-center gap-4">
          <div className="p-3 bg-blue-100 text-blue-600 rounded-lg"><FiCar size={24} /></div>
          <div><div className="text-sm text-slate-500">Active Vehicles</div><div className="text-xl font-bold text-slate-800">{stats.activeVehicles}</div></div>
        </div>
        <div className="bg-white p-6 rounded-xl border border-slate-100 shadow-sm flex items-center gap-4">
          <div className="p-3 bg-green-100 text-green-600 rounded-lg"><FiMapPin size={24} /></div>
          <div><div className="text-sm text-slate-500">Total Distance</div><div className="text-xl font-bold text-slate-800">{formatNumber(stats.totalDistance)} km</div></div>
        </div>
        <div className="bg-white p-6 rounded-xl border border-slate-100 shadow-sm flex items-center gap-4">
          <div className="p-3 bg-amber-100 text-amber-600 rounded-lg"><FiZap size={24} /></div>
          <div><div className="text-sm text-slate-500">Total Fuel</div><div className="text-xl font-bold text-slate-800">{formatNumber(stats.totalLitres)} L</div></div>
        </div>
        <div className="bg-white p-6 rounded-xl border border-slate-100 shadow-sm flex items-center gap-4">
          <div className="p-3 bg-rose-100 text-rose-600 rounded-lg"><FiDollarSign size={24} /></div>
          <div><div className="text-sm text-slate-500">Fuel Cost</div><div className="text-xl font-bold text-slate-800">{formatCurrency(stats.totalCost)}</div></div>
        </div>

        {stats.totalRentCost > 0 && (
          <div className="bg-white p-6 rounded-xl border border-slate-100 shadow-sm flex items-center gap-4">
            <div className="p-3 bg-purple-100 text-purple-600 rounded-lg"><FiDollarSign size={24} /></div>
            <div><div className="text-sm text-slate-500">Rent Cost</div><div className="text-xl font-bold text-slate-800">{formatCurrency(stats.totalRentCost)}</div></div>
          </div>
        )}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="bg-white p-6 rounded-xl border border-slate-100 shadow-sm">
          <h3 className="text-lg font-semibold text-slate-800 mb-4">Fuel Cost by Type</h3>
        ...
        <div className="lg:col-span-2 bg-white p-6 rounded-xl border border-slate-100 shadow-sm">
          <h3 className="text-lg font-semibold text-slate-800 mb-4">12-Month Fuel Consumption</h3>
          <div className="h-72">
      ...
      <div className="bg-white rounded-xl border border-slate-100 shadow-sm overflow-hidden">
        <div className="p-6 border-b border-slate-100">
          <h3 className="text-lg font-semibold text-slate-800">Top Performing Vehicles</h3>
        </div>
        <CustomTable
