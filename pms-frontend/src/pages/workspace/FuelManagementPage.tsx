...
const FuelManagementPage: React.FC = () => {
    ...
    const handleRefillFuelEntry = async () => {
        if (
            !selectedProjectId ||
            !refillForm.vehicleId ||
            !refillForm.openingKm ||
            !refillForm.supplierId ||
            !refillForm.litres ||
            !refillForm.pricePerLitre
        ) {
            toast.error("Please fill in all required fields");
            return;
        }
    ...
    const fuelColumns: ColumnDef<FuelEntry>[] = [
        ...
        },
        {
            ...
            header: "Unit Price",
            sortable: true,
            body: (row) => (row.pricePerLitre != null ? `₹${row.pricePerLitre.toFixed(2)}` : "—"),
        },
        { field: "litres", header: "Quantity", sortable: true, body: (row) => (row.litres != null ? `${row.litres.toFixed(2)} L` : "—") },
        {
            ...
            header: "Fuel Cost",
            sortable: true,
            body: (row) => (row.pricePerLitre != null && row.litres != null ? `₹${(row.pricePerLitre * row.litres).toFixed(2)}` : "—"),
        },
        ...
        },
        {
            ...
            header: "Mileage",
            sortable: true,
            body: (row) => (row.status === "CLOSED" && row.mileage != null ? row.mileage.toFixed(2) : "—"),
        },
        {
            field: "status",
            header: "Status",
                ...
                <Box sx={{ display: "flex", justifyContent: "center" }}>
                    {row.status === "OPEN" ? (
                        <IconButton
                            ...
                            onClick={() => {
                                setSelectedFuelEntry(row);
                                setShowCloseFuelDialog(true);
                            }}
                        >
                            <FiUnlock size={18} />
                        </IconButton>
        ...
        <Box sx={{ p: 2 }}>
            ...
            <Box sx={{ mb: 2, display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 2 }}>
                ...
                <Stack direction="row" spacing={1}>
                    ...
                    <CustomButton label="Refill" startIcon={<FiRefreshCw />} onClick={() => setShowRefillDialog(true)} variant="outlined" />
                </Stack>
            </Box>
            ...
            {fuelViewMode === "history" && (
                <Grid container spacing={2} sx={{ mb: 2 }}>
                    ...
                    <Grid size={{ xs: 12, sm: 6, md: 3 }}>
                        <CustomDateInput label="From Date" value={fuelDateFrom} onChange={(date) => setFuelDateFrom(date)} />
                    </Grid>
                    <Grid size={{ xs: 12, sm: 6, md: 3 }}>
                        <CustomDateInput label="To Date" value={fuelDateTo} onChange={(date) => setFuelDateTo(date)} />
                    </Grid>
                </Grid>
            )}
            ...
            {/* Fuel Table */}
            <CustomTable data={filteredFuelEntries} columns={fuelColumns} loading={loading} />
            ...
            <CustomModal
                ...
                footer={
                    <>
                        ...
                        <Button variant="contained" onClick={handleAddFuelEntry}>
                            Add Entry
                        </Button>
                    </>
                }
                ...
                <Grid container spacing={2}>
                    <Grid size={{ xs: 12, sm: 6 }}>
                        <CustomDateInput
                            label="Date"
                            value={fuelForm.date}
                            onChange={(date) => setFuelForm({ ...fuelForm, date: date || new Date() })}
                        />
                    </Grid>
                    <Grid size={{ xs: 12, sm: 6 }}>
                        <CustomSelect
                    ...
                    <Grid size={{ xs: 12, sm: 6 }}>
                        <CustomSelect
                            label="Supplier"
                            value={fuelForm.supplierId}
                            onChange={(value) => setFuelForm({ ...fuelForm, supplierId: value })}
                    ...
                    <Grid size={{ xs: 12, sm: 6 }}>
                        <CustomTextField
                            ...
                            onChange={(e) => setFuelForm({ ...fuelForm, openingKm: e.target.value })}
                            required
                        />
                    </Grid>
                    <Grid size={{ xs: 12, sm: 6 }}>
                        <CustomTextField
                            ...
                            onChange={(e) => setFuelForm({ ...fuelForm, pricePerLitre: e.target.value })}
                            required
                        />
                    </Grid>
                </Grid>
            ...
            <CustomModal
                ...
                footer={
                    <>
                        ...
                        <Button variant="contained" onClick={handleCloseFuelEntry}>
                            Close Entry
                        </Button>
                    </>
                }
                    ...
                    <Grid container spacing={2}>
                        <Grid size={{ xs: 12 }}>
                            <Typography variant="body2">
                                <strong>Vehicle:</strong> {selectedFuelEntry.vehicleName}
                            </Typography>
                            <Typography variant="body2">
                                <strong>Opening KM:</strong> {selectedFuelEntry.openingKm.toFixed(1)}
                            </Typography>
                        </Grid>
                        <Grid size={{ xs: 12 }}>
                            <CustomTextField label="Closing KM" type="number" value={closingKm} onChange={(e) => setClosingKm(e.target.value)} required />
                        </Grid>
                    </Grid>
                )}
            </CustomModal>
            ...
            <CustomModal
                ...
                footer={
                    <>
                        ...
                        <Button variant="contained" onClick={handleRefillFuelEntry}>
                            Record Refill
                        </Button>
                    </>
                }
                ...
                <Grid container spacing={2}>
                    <Grid size={{ xs: 12, sm: 6 }}>
                        <CustomDateInput
                            ...
                            onChange={(date) => setRefillForm({ ...refillForm, date: date || new Date() })}
                        />
                    </Grid>
                    <Grid size={{ xs: 12, sm: 6 }}>
                        <CustomSelect
                            ...
                            onChange={(value) => setRefillForm({ ...refillForm, vehicleId: value })}
                            options={vehicleOptions}
                            required
                        />
                    </Grid>
                    ...
                    <Grid size={{ xs: 12, sm: 6 }}>
                        <CustomSelect
                            ...
                            onChange={(value) => setRefillForm({ ...refillForm, supplierId: value })}
                            options={supplierOptions}
                            required
                        />
                    </Grid>
